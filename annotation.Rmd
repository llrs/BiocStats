...
Here we are going to analyse the Annotation packages of Bioconductor. See the home of the analysis [here](index.html).
...
```
Packages tend to stay up to 8 years. Not surprisingly the number of packages incorporated before 2009 and still in the repository are of  `r nrow(df) - nrow(deprecation)` packages. But those packages not removed how do they do in Bioconductor?
...
ggplot(model[Package %in% packages], aes(i.rank, rank, col = Package)) +
  geom_point() + 
  theme_bw + 
  geom_line() +
...
https://talks.cpsievert.me/20170125/demo.html
```
