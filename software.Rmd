...
Here we are going to analyse the software packages of Bioconductor. See the home of the analysis [here](index.html).
...
There have been `r length(unique(stats$Package))` Software packages in Bioconductor. Some have been added recently and some later.
...
```{r timePackage, fig.cap="Time of packages between first and last download."}
...
ratio
```
We can see that the package with more downloads from the same IP is `r Biocpkg(ratio[1, 1])`, followed by, `r Biocpkg(ratio[2, 1])`, `r Biocpkg(ratio[3, 1])` and the forth one is `r Biocpkg(ratio[4, 1])`. AT the moment I last edited this manually, the first one is for Chip-seq, the second one for flow cytometry, and the third  and forth one is for chromatographically separated and single-spectra mass spectral data, maybe few locations use these packages.
