...
Here we are going to analyse the software packages of Bioconductor. See the home of the analysis [here](index.html) were we already transormated the data. From that stats we are going to analyse the software category:
...
```{r normalize1, fig.cap = "Position of packages in Bioconductor"}
...
```{r normalize2, fig.cap = "% of packages in Bioconductor. "}
...
```{r cycle1, fig.cap = "Cycle of packages. Position of package downloads respect the maximum downloads of the packages along time."}
...
ggplot(norm4[Package %in% c(packages, "RTools4TB", "SemSim"), ], 
  ...
  theme
```
As expected the packages that keep up with Bioconductor growth have a peak near the end of the serie. For this reason I added the package `r Biocpkg("SemSim")` and the `r Biocpkg("RTools4TB")` to see that package that has been less and less downloaded.
